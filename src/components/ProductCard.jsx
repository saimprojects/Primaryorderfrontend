...
const ProductCard = ({ product }) => {
    ...
    const calculateDiscount = (product) => {
        ...
        // 🔥 UPDATED: Check for variants
        if (product.has_variants) {
            // Use display price or min price for variant products
    ...
    // 🔥 UPDATED: Handle add to cart with variant warning
    const handleAddToCart = async (e) => {
        e.preventDefault();
        e.stopPropagation();
        
        if (hasVariants) {
            // For variant products, redirect to product detail page
    ...
    // 🔥 UPDATED: Final price to display
    const finalPrice = discount ? discount.discountedPrice : 
                     hasVariants ? (product.display_price || product.min_price || 0) : 
                    ...
                    <div className="mb-4">
                        ...
                        {hasVariants && product.variants && product.variants.length > 1 && (
                            <div className="text-sm text-gray-500 mt-1">
                                From {formatPrice(product.min_price || 0)}
                            </div>
                        )}
